...
if ($null -ne $configuration -and $configuration -ne '') {
    $restoreCommand += "--configuration $configuration "
}

if ($null -ne $disableParallel -and $disableParallel -ne $false) {
    $restoreCommand += "--disable-parallel "
}

if ($null -ne $force -and $force -ne $false) {
    $restoreCommand += "--force "
}

if ($null -ne $forceEvaluate -and $forceEvaluate -ne $false) {
    $restoreCommand += "--force-evaluate "
}

if ($null -ne $noCache -and $noCache -ne $false) {
    $restoreCommand += "--no-cache "
}

if ($null -ne $noDependencies -and $noDependencies -ne $false) {
    $restoreCommand += "--no-dependencies "
}

if ($null -ne $verbosity -and $verbosity -ne '') {
    $restoreCommand += "--verbosity $verbosity"
}
